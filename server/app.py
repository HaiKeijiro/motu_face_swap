...
class GoogleDriveManager:
    """Manages Google Drive operations for uploading and sharing images."""
    ...
    def _setup_gallery_folder(self) -> None:
        """Set up or find the Face Swap Gallery folder in Google Drive."""
        try:
            if not self.service:
                raise Exception("Google Drive service not initialized")
            
            # Search for existing Face Swap Gallery folder
            results = self.service.files().list(
                q="name='Face Swap Gallery' and mimeType='application/vnd.google-apps.folder' and trashed=false",
            ...
            if folders:
                ...
                logger.info(f"Using existing Face Swap Gallery folder: {self.folder_id}")
            else:
                # Create new folder
                folder_metadata = {
                    'name': 'Face Swap Gallery',
                ...
                self._make_folder_public(self.folder_id)
                
                logger.info(f"Created new Face Swap Gallery folder: {self.folder_id}")
                
        except Exception as e:
            logger.error(f"Failed to setup gallery folder: {e}")
            raise
    ...
    def _make_folder_public(self, folder_id: str) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to make folder public: {e}")
    
    def upload_image(self, image_data: bytes, filename: str) -> Optional[Dict[str, str]]:
        """
        Upload an image to Google Drive and return public sharing info.
        ...
        Returns:
            Dict containing file_id, public_url, and gallery_url
        """
        try:
            if not self.service or not self.folder_id:
                raise Exception("Google Drive service or folder not initialized")
            ...
            try:
                ...
                gallery_url = f"https://drive.google.com/drive/folders/{self.folder_id}"
    ...
    def generate_qr_code(self, url: str) -> Optional[bytes]:
        """
...
class FaceSwapApp:
    ...
    def _setup_routes(self) -> None:
        ...
        def swap_face():
            ...
            try:
                ...
                try:
                    ...
                    if result_data:
                        # Upload to Google Drive and generate QR code
                        upload_info = None
                        qr_code_data = None
                        ...
                        if self.google_drive_manager:
                            try:
                                ...
                                filename = f"faceswap_{int(time.time())}_{uuid.uuid4().hex[:8]}.jpg"
                                
                                # Upload image to Google Drive
                                upload_info = self.google_drive_manager.upload_image(result_data, filename)
                                
                                if upload_info:
                                    # Generate QR code for the public gallery link
                                    qr_code_data = self.google_drive_manager.generate_qr_code(upload_info['gallery_url'])
                                    logger.info(f"Successfully uploaded image and generated QR code: {filename}")
                                else:
                                    logger.warning("Failed to upload image to Google Drive")
                                    
                            except Exception as drive_error:
                                logger.error(f"Google Drive operation failed: {drive_error}")
                                # Continue without Google Drive functionality
                        else:
                            logger.info("Google Drive integration not available")
                        
                        response_data = {
                            'image': base64.b64encode(result_data).decode('utf-8')
                        }
                        
                        # Add Google Drive information if available
                        if upload_info:
                            response_data['google_drive'] = upload_info
                            
                        if qr_code_data:
                            response_data['qr_code'] = base64.b64encode(qr_code_data).decode('utf-8')
        ...
        def upload_image():
            """Upload an image to Google Drive and generate QR code."""
            try:
                ...
                image = request.files['image']
                filename = request.form.get('filename', f"image_{uuid.uuid4()}.jpg")
                ...
                try:
                    # Upload image to Google Drive
                    upload_info = self.google_drive_manager.upload_image(open(temp_path, 'rb').read(), filename)
                    
                    if upload_info:
                        # Generate QR code
                        qr_code_data = self.google_drive_manager.generate_qr_code(upload_info['public_url'])
        ...
        @self.app.route('/api/google-drive/status', methods=['GET'])
            ...
            try:
                if self.google_drive_manager:
                    return jsonify({
                        ...
                        'folder_id': self.google_drive_manager.folder_id
                    })
                else:
                    return jsonify({
                        'status': 'disabled',
